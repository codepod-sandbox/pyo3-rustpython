...
impl ValidationOptions<Arc<dyn referencing::Retrieve>> {
    ...
    /// Stub for when resolve-http is not enabled. Returns Ok(self) — HTTP
    /// options are silently ignored since reqwest is not available.
    #[cfg(not(all(feature = "resolve-http", not(target_arch = "wasm32"))))]
    pub fn with_http_options(
        self,
        _options: &crate::HttpOptions,
    ) -> Result<Self, crate::retriever::HttpRetrieverError> {
        Ok(self)
    }
